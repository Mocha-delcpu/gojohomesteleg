...
exports.searchPropertyWizard = void 0;
...
exports.searchPropertyWizard = new telegraf_1.Scenes.WizardScene('SEARCH_PROPERTY_SCENE', 
// ── Step 1: Ask Location ─────────────────────────────────────────────────
async (ctx) => {
    const t = i18n_1.i18n.get(ctx.session?.language);
    await ctx.reply(t.searchStep1, { parse_mode: 'Markdown', ...(0, menus_1.getCancelMenu)(ctx.session?.language) });
    ctx.wizard.state = {};
...
async (ctx) => {
    ...
    await ctx.reply(t.searchStep3, { parse_mode: 'Markdown', ...(0, menus_1.getCancelMenu)(ctx.session?.language) });
    return ctx.wizard.next();
}, 
...
async (ctx) => {
    ...
    await ctx.reply(t.searchStep4, { parse_mode: 'Markdown', ...(0, menus_1.getCancelMenu)(ctx.session?.language) });
    return ctx.wizard.next();
}, 
...
async (ctx) => {
    ...
    await ctx.reply(t.searchSearching, telegraf_1.Markup.removeKeyboard());
    const s = ctx.wizard.state;
    const location = s.location;
    const type = s.propertyType;
    const minPrice = ctx.wizard.state.minPrice;
    // Search Supabase
    const results = await (0, db_1.searchProperties)(location, type, minPrice, maxPrice);
    if (results.length === 0) {
        await ctx.reply(t.searchNoResults, { parse_mode: 'Markdown', ...telegraf_1.Markup.removeKeyboard() });
    }
    else {
    ...
    const searchLink = (0, formatting_1.buildSearchLink)(location, minPrice, maxPrice, type);
    const shareText = encodeURIComponent(`🔍 Check out properties in ${location} on Gojo Homes!\n${searchLink}`);
    const telegramShareUrl = `https://t.me/share/url?url=${encodeURIComponent(searchLink)}&text=${shareText}`;
    await ctx.reply(`🔗 *Share this search* with friends or in Telegram groups:`, {
        parse_mode: 'Markdown',
        reply_markup: {
            inline_keyboard: [
    ...
    logger_1.logger.info(`Search completed: location=${location}, type=${type}, price=${minPrice}-${maxPrice}, results=${results.length}`);
    return ctx.scene.leave();
});
const cancelSearch = async (ctx) => {
    const t = i18n_1.i18n.get(ctx.session?.language);
    await ctx.reply('❌ Search cancelled. Use /start to go back to the main menu.', telegraf_1.Markup.removeKeyboard());
    return ctx.scene.leave();
};
